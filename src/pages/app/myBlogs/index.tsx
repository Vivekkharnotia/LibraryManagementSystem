...
import BlogsCard from "components/blogs/Blogs/BlogCard";
...
const App = ({ metaBlogsDataString }: { metaBlogsDataString: string }) => {
  ...
  const handleDelete = async () => {
    ...
    metaBlogsData.splice(
      metaBlogsData.findIndex((blog: any) => blog.id === blogId),
      1
    );
  ...
  const handlePublishToggle = async () => {
    try {
      ...
      metaBlogsData.find((blog: MetaBlog) => blog.id === blogId)!.published =
        !published;
      setMetaBlogsData([...metaBlogsData]);
    ...
    <>
      ...
      >
        <Box
          sx={{
            position: "relative",
            "&:hover>a>svg": { right: "2px" },
            maxWidth: "max-content",
            margin: "auto",
            "@media (max-width:250px)": { fontSize: "0.7rem" },
          }}
        >
          <Link
            href="/blogs/create"
          ...
          {metaBlogsData.map((blog: MetaBlog) => (
            ...
            >
              <Link href={`/blogs/read/${blog.id}`} passHref>
                <BlogsCard
                  imgUrl={blog.heroImageSrc}
                  title={blog.headTitle}
                  mainTitle={blog.published ? "Published" : "Not Published"}
                  blogId={blog.id}
                />
              </Link>
              ...
              <IconButton
                ...
                onClick={(e) => handleClick(e, blog.id, blog.published)}
                sx={{
                  position: "absolute",
                  top: "0",
                  right: "-1.8rem",
